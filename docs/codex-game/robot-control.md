...
- T2 wheels use functional `WheelA1_01..04` and `WheelA2_01..07` parent pivots on each side. Child pivots keep only Unity debris components; one root `TrackedVehicleAnimator` owns left/right wheel arrays, per-wheel rotation speeds, and both track renderers.
